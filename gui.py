...
class App:
    ...
    def __init__(self,root_window):
       ...
       # 認証ファイルのラベルとボタン
        ...
        self.auth_button = tk.Button(self.root,text="Select",command=self.select_auth_file)
    ...
    # 実行ボタン
        self.auth_file_path = ""
        self.run_button = tk.Button(self.root,text="Run Scraper",command=self.run_scraper)
    ...
    def select_auth_file(self):
        ...
        self.auth_file_path = filedialog.askopenfilename(title="Select Credentials file")
        if self.auth_file_path:
            messagebox.showinfo("Success",f"Credentials file selected:\n{self.auth_file_path}")
    def run_scraper(self):
        ...
        if not self.auth_file_path or not sheet_id:
            messagebox.showerror("Error","select a Credentials file and enter a Spreadsheet ID")
            return
